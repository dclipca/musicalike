  ...
  render() {
    return (
            <ListItem key={this.props.videoId} thumbnail>
              <Left >
                <Thumbnail scaleX={1.30} scaleY={1.30} square source={{ uri: 'https://img.youtube.com/vi/' + this.props.videoId + '/mqdefault.jpg' }} />
              </Left>
              <Body>
                <Text>{this.props.name}</Text>
                <Text note numberOfLines={1}>Match: {Math.round(this.props.match * 100)}%</Text>
                {this.props.isShown ?
                <YouTube
              ...
              <Right>
                <Button transparent>
                  <TouchableOpacity onPress={this.props.reverseIsShowForParticualArtistListItem}>
...
styles = StyleSheet.create({
  youtube : {
    alignSelf: 'stretch', height: height * 4/10, flex: 1
  }
})
    ...
    return {
        reverseIsShowForParticualArtistListItem: () => {
            ...
            getSimilarResponseClone.forEach((artist) => {
              ...
              dispatch({type: 'REVERSE_IS_SHOW_FOR_PARTICULAR_ARTIST_LIST_ITEM', payload: getSimilarResponseClone})
            })
        }
