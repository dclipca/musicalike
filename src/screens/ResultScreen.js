...
class ResultScreen extends React.Component {
    ...
    getSimilar = (suggestedArtistName = this.props.suggestedArtistName) => {
      fetch(
        ...
        .then(getSimilarResponse => {
          ...
          getSimilarResponse.similarartists.artist
            .slice(0, 8)
            .forEach(artistArray => {
          ...
          modifiedGetSimilarResponse.forEach(artist => {
            YTSearch(
              {
                key: "AIzaSyCIjfFKB5LuYPf4LB6gVhFVWnKZHejp45s",
                term: artist["name"],
                maxResults: 1
              },
              searchResult => {
        ...
        .then(() => {
          setTimeout(() => {
            this.props.sendGetSimilarResponseToState(
              modifiedGetSimilarResponse
            );
            console.log(modifiedGetSimilarResponse);
          }, 1000);
        });
    };
    getSimilar();
  ...
  render() {
    return this.props.resultsReady ? (
      <Container>
        <Content>
          <List>
...
function mapDispatchToProps(dispatch) {
  return {
    sendGetSimilarResponseToState: getSimilarResponse =>
      dispatch({
        type: "SEND_GET_SIMILAR_RESPONSE_TO_STATE",
        payload: getSimilarResponse
      })
  };
}
