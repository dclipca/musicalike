...
class MainScreen extends React.Component {
  getCorrection = artistName => {
    fetch(
      "http://ws.audioscrobbler.com/2.0/?method=artist.getcorrection&artist=" +
        artistName +
        "&api_key=ab8caed470fc3b1760a688cb40f78a2b&format=json"
    )
      ...
      .then(getCorrectionResponse => {
        ...
        if (
          artistNameCorrection != undefined &&
          artistNameCorrection != this.props.formValue
        ) {
...
function mapDispatchToProps(dispatch) {
  return {
    resetResultsReady: () =>
      dispatch({ type: "RESET_RESULTS_READY", payload: false }),
    changeFormValue: newValue =>
      dispatch({ type: "CHANGE_FORM_VALUE", payload: newValue }),
    changeSuggestedArtistName: newArtistName =>
      dispatch({
        type: "CHANGE_SUGGESTED_ARTIST_NAME",
        payload: newArtistName
      }),
    changeConfirmationRequired: boolean =>
      dispatch({ type: "CHANGE_CONFIRMATION_REQUIRED", payload: boolean })
  };
}
